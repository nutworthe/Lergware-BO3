...
void ImGui::TextUnformatted(const std::string& text, const char* text_end)
{
    TextEx(text.data(), text_end, ImGuiTextFlags_NoWidthForLargeClippedText);
}
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    else if (*v)
    {
        ...
        const float pad = ImMax(1.0f, (float)(int)(check_sz / 18.0f));
...
bool ImGui::InputTextWithHint(const std::string& label, const char* hint, std::string* buf, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data)
{
    IM_ASSERT((flags & ImGuiInputTextFlags_CallbackResize) == 0);
    flags |= ImGuiInputTextFlags_CallbackResize;
    ...
    cb_user_data.ChainCallback = callback;
    cb_user_data.ChainCallbackUserData = user_data;
    return InputTextEx(label.data(), hint, (char*)buf->c_str(), buf->capacity() + 1, {}, flags, InputTextCallback, &cb_user_data);
}
...
bool ImGui::Selectable(const std::string& label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    ImVec2 label_size = CalcTextSize(label.data(), NULL, true);
    ...
    RenderTextClipped(bb_inner.Min, bb_inner.Max, label.data(), NULL, &label_size, style.SelectableTextAlign, &bb);
    ...
    IMGUI_TEST_ENGINE_ITEM_INFO(id, label.data(), window->DC.ItemFlags);
    return pressed;
}

bool ImGui::Selectable(const std::string& label, bool* p_selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
...
bool ImGui::BeginMenu(const std::string& label, bool enabled)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal)
    {
        ...
        pressed = Selectable(label.data(), menu_is_open, ImGuiSelectableFlags_NoHoldingActiveID | ImGuiSelectableFlags_PressedOnClick | ImGuiSelectableFlags_DontClosePopups | (!enabled ? ImGuiSelectableFlags_Disabled : 0), ImVec2(w, 0.0f));
        PopStyleVar();
    ...
    else
    {
        ...
        pressed = Selectable(label.data(), menu_is_open, ImGuiSelectableFlags_NoHoldingActiveID | ImGuiSelectableFlags_PressedOnClick | ImGuiSelectableFlags_DontClosePopups | ImGuiSelectableFlags_DrawFillAvailWidth | (!enabled ? ImGuiSelectableFlags_Disabled : 0), ImVec2(w, 0.0f));
    ...
    IMGUI_TEST_ENGINE_ITEM_INFO(id, label.data(), window->DC.ItemFlags | ImGuiItemStatusFlags_Openable | (menu_is_open ? ImGuiItemStatusFlags_Opened : 0));

    if (!menu_is_open && want_open && g.OpenPopupStack.Size > g.BeginPopupStack.Size)
    {
        // Don't recycle same menu level in the same frame, first close the other menu and yield for a frame.
        OpenPopup(label.data());
        return false;
    }
...
bool ImGui::MenuItem(const std::string& label, const char* shortcut, bool selected, bool enabled)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal)
    {
        ...
        window->DC.CursorPos.x += IM_FLOOR(style.ItemSpacing.x * 0.5f);
        PushStyleVar(ImGuiStyleVar_ItemSpacing, ImVec2(style.ItemSpacing.x * 2.0f, style.ItemSpacing.y));
        pressed = Selectable(label.data(), false, flags, ImVec2(w, 0.0f));
        PopStyleVar();
        window->DC.CursorPos.x += IM_FLOOR(style.ItemSpacing.x * (-1.0f + 0.5f)); // -1 spacing to compensate the spacing added when Selectable() did a SameLine(). It would also work to call SameLine() ourselves after the PopStyleVar().
    }
    else
    {
        ...
        float extra_w = ImMax(0.0f, GetContentRegionAvail().x - w);
        pressed = Selectable(label.data(), false, flags | ImGuiSelectableFlags_DrawFillAvailWidth, ImVec2(w, 0.0f));
    ...
    IMGUI_TEST_ENGINE_ITEM_INFO(window->DC.LastItemId, label.data(), window->DC.ItemFlags | ImGuiItemStatusFlags_Checkable | (selected ? ImGuiItemStatusFlags_Checked : 0));
    return pressed;
}

bool ImGui::MenuItem(const std::string& label, const char* shortcut, bool* p_selected, bool enabled)
{
